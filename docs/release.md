...
- The npm `version` lifecycle also regenerates the F-Droid changelog files from `CHANGELOG.md` (`npm run fdroid:changelogs`) and stages them, so the release tag carries them. It **aborts the release** if `CHANGELOG.md` has no entry for the version being cut — commit the changelog entry first, as the checklists require. See [docs/android.md](android.md) for why these files are generated per ABI.
